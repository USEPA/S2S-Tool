...
def dfappend_saprc07tc_ae8(dfin):
  '''
  ...
  for idx, row in dfin.iterrows():
    ...
    else: mechspecies = get_saprc07tc_ae8_roc(smiles,log10cstar,koh)
...
def get_saprc07tc_ae8_roc(smiles,log10cstar,koh):
  '''
  Function maps input reactive organic carbon (ROC) species to SAPRC07TC_AE8 species.
  ...
  # Lumped species
  # Volatility defined
  elif ( log10cstar < -0.5 ):       mechspecies = 'SVOCN1' # log(C*) < -0.5
  elif ( log10cstar < 0.5 ):        mechspecies = 'SVOCP0' # -0.5 < log(C*) < 0.5
  elif ( log10cstar < 1.5 ):        mechspecies = 'SVOCP1' # 0.5 < log(C*) < 1.5
  elif ( log10cstar < 2.5 ):        mechspecies = 'SVOCP2' # 1.5 < log(C*) < 2.5
  elif ( log10cstar < 3.5 ):        mechspecies = 'IVOCP3' # 2.5 < log(C*) < 3.5
  elif ( log10cstar < 4.5 ):        mechspecies = 'IVOCP4' # 3.5 < log(C*) < 4.5
  elif ( log10cstar < 5.5 and nbenzene > 0):
                                    mechspecies = 'IVOCP5ARO' # 4.5 < log(C*) < 5.5; Aromatic
  elif ( log10cstar < 5.5 ):        mechspecies = 'IVOCP5'    # 4.5 < log(C*) < 5.5
  elif ( log10cstar < 6.5 and nbenzene > 0):
                                    mechspecies = 'IVOCP6ARO' # 5.5 < log(C*) < 6.5; Aromatic
  elif ( log10cstar < 6.5 ):        mechspecies = 'IVOCP6'    # 5.5 < log(C*) < 6.5
