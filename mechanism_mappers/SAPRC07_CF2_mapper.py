...
def dfappend_saprc07_cf2(dfin):
  ...
  for idx, row in dfin.iterrows():
    ...
    if ( pd.isnull(smiles) or smiles=='-' or pd.isnull(koh) ):
                                    mechspecies = 'NROG' # Unreactive/nonreactive
    else: mechspecies = get_saprc07_cf2_roc(smiles,log10cstar,koh)
...
def get_saprc07_cf2_roc(smiles,log10cstar,koh):
  '''
  Function maps input reactive organic carbon (ROC) species to SAPRC07_CF2 species.
